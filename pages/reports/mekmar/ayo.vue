<template>
  <div>
        ...
        <table class="table caption-top">
          ...
          <tbody>
            <tr>
              ...
              <td>
                {{
                  (getReportsMekmarAyoListTotal.profitUsd - cost_usd)
                    | formatPriceUsd
                }}
              </td>
              <td>
                {{
                  (getReportsMekmarAyoListTotal.profitTl - cost_tl)
                    | formatPriceTl
                }}
              </td>
            </tr>
    ...
    <div class="container row">
      <div class="col">
        <Dropdown
          ...
          @change="quarterSelected($event)"
        />
      </div>
      <div class="col">
        <Dropdown
          class="w-100"
          v-model="selectedHalfMonths"
          :options="halfmonths"
          optionLabel="month"
          placeholder="Select a Month"
          @change="halfMonthsSelected($event)"
        />
      </div>
      <div class="col">
        <Button
          ...
          label="Credit Card"
          @click="creditCardToggle"
        />
        <Dialog
          :visible.sync="credit_card_form_visible"
          header="Credit Card Cost"
          modal
        >
          <creditCardForm
            style="margin-bottom: 5px"
            v-for="item in creditCardCostList"
            :key="item"
            :month="item.month"
            :value="item.value"
      ...
      <div class="col">
        ...
        />
        <Dialog
          :visible.sync="travel_cost_form_visible"
          header="Travel Cost"
          modal
        >
          <creditCardForm
            style="margin-bottom: 5px"
            v-for="item in travelCostList"
            :key="item"
            :month="item.month"
            :value="item.value"
            :month_id="item.month_id"
      ...
      <div class="col">
        <Button
          type="button"
          class="p-button-help w-100"
          label="Sample Cost"
          @click="sampleCost"
        />
        <Dialog
          :visible.sync="sample_cost_form_visible"
          header="Sample Cost"
          modal
        >
          <creditCardForm
            style="margin-bottom: 5px"
            v-for="item in sampleCostList"
            :key="item"
            :month="item.month"
            :value="item.value"
            :month_id="item.month_id"
      ...
      <div class="col">
        ...
        <Dialog
          :visible.sync="other_cost_form_visible"
          header="Fuil Cost"
          modal
        >
          <creditCardForm
            style="margin-bottom: 5px"
            v-for="item in otherCostList"
            :key="item"
            :month="item.month"
            :value="item.value"
            :month_id="item.month_id"
      ...
      <div class="col">
        ...
        <Dialog
          :visible.sync="mekmer_cost_form_visible"
          header="Meal Cost"
          modal
        >
          <creditCardForm
            style="margin-bottom: 5px"
            v-for="item in mekmerCostList"
            :key="item"
            :month="item.month"
            :value="item.value"
            :month_id="item.month_id"
    ...
    <Dialog
      :visible.sync="mekmar_ayo_detail_dialog"
...
export default {
  ...
  methods: {
    ...
    mekmerCost() {
      this.$axios
        .get(`/reports/ayo/mekmer/cost/list/${this.selectedYear.Yil}`)
        .then((res) => {
    ...
    mekmerCostUpdate(event) {
      this.$axios.put("/reports/ayo/mekmer/cost/update", event).then((res) => {
        this.$toast.success("Başarıyla Güncellendi.");
      });
    },
    mekmerCostSave(event) {
      this.$axios.post("/reports/ayo/mekmer/cost/save", event).then((res) => {
    ...
    cost_excel() {
      this.$axios
        .get(`/reports/ayo/other/cost/list/${this.selectedYear.Yil}`)
        .then((other) => {
          this.$axios
            .get(`/reports/ayo/sample/cost/list/${this.selectedYear.Yil}`)
            .then((sample) => {
              this.$axios
                .get(`/reports/ayo/wage/cost/list/${this.selectedYear.Yil}`)
                .then((wage) => {
                  this.$axios
                    .get(
                      `/reports/ayo/travel/cost/list/${this.selectedYear.Yil}`
                    )
                    .then((travel) => {
                      this.$axios
                        .get(
                          `/reports/ayo/credit/card/cost/list/${this.selectedYear.Yil}`
                        )
                        .then((credit) => {
                          this.$axios
                            .get(
                              `/reports/ayo/mekmer/cost/list/${this.selectedYear.Yil}`
                            )
                            .then((meal) => {
                              const data = {
    ...
    updatedCurrencyList() {
      this.$axios
        .get(`/reports/ayo/currency/list/${this.selectedYear.Yil}`)
        .then((res) => {
          this.currency_list = res.data.list;
        });
    },
    updateCurrency(event) {
      this.$axios.put("/reports/ayo/currency/update", event).then((res) => {
        this.$toast.success("Başarıyla Güncellendi.");
      });
    },
    saveCurrency(event) {
      this.$axios.post("/reports/ayo/currency/save", event).then((res) => {
    ...
    __getHalfCostList(month_1, month_2, year) {
      this.$axios
        .get(`/reports/ayo/costs/half/${month_1}/${month_2}/${year}`)
        .then((res) => {
    ...
    __getQuarterCostList(quarter_1, quarter_2, year) {
      this.$axios
        .get(`/reports/ayo/costs/quarter/${quarter_1}/${quarter_2}/${year}`)
        .then((res) => {
          this.cost_tl = res.data.tl;
          this.cost_usd = res.data.usd;
        });
    ...
    __getMonthlyCostList(year, month) {
      this.$axios
        .get(`/reports/ayo/costs/list/${year}/${month}`)
        .then((res) => {
          this.cost_tl = res.data.tl;
          this.cost_usd = res.data.usd;
        });
    ...
    __getYearlyCostList(year) {
      this.$axios.get(`/reports/ayo/costs/yearly/${year}`).then((res) => {
        this.cost_tl = res.data.tl;
        this.cost_usd = res.data.usd;
      });
    },
    ...
    otherCostUpdate(event) {
      this.$axios.put("/reports/ayo/other/cost/update", event).then((res) => {
        this.$toast.success("Başarıyla Güncellendi.");
      });
    },
    otherCostSave(event) {
      this.$axios.post("/reports/ayo/other/cost/save", event).then((res) => {
        this.$toast.success("Başarıyla Kaydedildi.");
    ...
    sampleCostUpdate(event) {
      this.$axios.put("/reports/ayo/sample/cost/update", event).then((res) => {
        this.$toast.success("Başarıyla Güncellendi.");
      });
    },
    sampleCostSave(event) {
      this.$axios.post("/reports/ayo/sample/cost/save", event).then((res) => {
        this.$toast.success("Başarıyla Kaydedildi.");
    ...
    abroadCostListUpdated() {
      this.$axios
        .get(`/reports/ayo/abroad/cost/list/${this.selectedYear.Yil}`)
        .then((res) => {
          this.abroadCostList = res.data.list;
        });
    },
    abroadCostUpdate(event) {
      this.$axios.put("/reports/ayo/abroad/cost/update", event).then((res) => {
        this.$toast.success("Başarıyla Güncellendi.");
      });
    },
    abroadCostSave(event) {
      this.$axios.post("/reports/ayo/abroad/cost/save", event).then((res) => {
        this.$toast.success("Başarıyla Kaydedildi.");
    ...
    wageCostUpdate(event) {
      this.$axios.put("/reports/ayo/wage/cost/update", event).then((res) => {
        this.$toast.success("Başarıyla Güncellendi.");
      });
    },
    wageCostSave(event) {
      this.$axios.post("/reports/ayo/wage/cost/save", event).then((res) => {
    ...
    travelCostUpdate(event) {
      this.$axios.put("/reports/ayo/travel/cost/update", event).then((res) => {
        this.$toast.success("Başarıyla Güncellendi.");
      });
    },
    travelCostSave(event) {
      this.$axios.post("/reports/ayo/travel/cost/save", event).then((res) => {
        this.$toast.success("Başarıyla Kaydedildi.");
    ...
    creditCardCostUpdate(event) {
      this.$axios
        .put("/reports/ayo/credit/card/cost/update", event)
        .then((res) => {
          this.$toast.success("Başarıyla Güncellendi.");
        });
    },
    creditCardCostSave(event) {
      this.$axios
        .post("/reports/ayo/credit/card/cost/save", event)
        .then((res) => {
          this.$toast.success("Başarıyla Kaydedildi.");
    ...
    travelCostListUpdated() {
      this.$axios
        .get(`/reports/ayo/travel/cost/list/${this.selectedYear.Yil}`)
        .then((res) => {
    ...
    creditCardCostListUpdated() {
      this.$axios
        .get(`/reports/ayo/credit/card/cost/list/${this.selectedYear.Yil}`)
        .then((res) => {
      ...
      if (event.value.id == 0) {
        ...
        api
          ...
          .then((response) => {
            if (response) {
              ...
              this.$axios
                .get(
                  `/maliyet/proforma/currency/half/${
                    this.selectedYear.Yil
                  }/${1}/${6}`
                )
                .then((res) => {
                  this.profitProformaTl =
                    this.getReportsMekmarAyoListTotal.proforma *
                    res.data.currency;
                });
            }
          });
        ...
        api
          ...
          .then((response) => {
            ...
            this.$axios
              .get(`/maliyet/proforma/all/currency/${this.selectedYear.Yil}`)
              .then((res) => {
                this.profitProformaTl =
                  this.getReportsMekmarAyoListTotal.proforma *
                  res.data.currency;
      ...
      if (event.value.id == 0) {
        ...
        api
          ...
          .then((response) => {
            ...
            this.$store.commit("setReportsMekmarAyoListTotal", response.data);
            this.$store.dispatch("setEndLoadingAction");
            this.__getMonthlyCostList(
              this.selectedYear.Yil,
              this.getReportsMekmarAyoMonthList[0].Ay
            );
            this.selectedMonth = this.getReportsMekmarAyoMonthList[0];
            this.$axios
              .get(
                `/maliyet/proforma/currency/${this.selectedYear.Yil}/${this.getReportsMekmarAyoMonthList[0].Ay}`
              )
              .then((res) => {
                this.profitProformaTl =
                  this.getReportsMekmarAyoListTotal.proforma *
        ...
        api
          ...
          .then((response) => {
            if (response) {
              ...
              this.$axios
                .get(
                  `/maliyet/proforma/currency/quarter/${
                    this.selectedYear.Yil
                  }/${1}/${3}`
                )
                .then((res) => {
      ...
      api
        ...
        .then((response) => {
          if (response.status) {
            const link = document.createElement("a");
            link.href =
              this.getLocalUrl + "maliyet/dosyalar/maliyetRaporExcelListe";
    ...
    yearSelected(event) {
      ...
      api
        ...
        .then((response) => {
          ...
          this.selectedQuarter = { quarter: "All", id: 0 };
          this.$axios
            .get(`/maliyet/proforma/currency/${event.value.Yil}/${12}`)
            .then((res) => {
              this.profitProformaTl =
                this.getReportsMekmarAyoListTotal.proforma * res.data.currency;
            });
        });
    },
    monthSelected(event) {
      ...
      api
        ...
        .then((response) => {
          ...
          this.selectedQuarter = { quarter: "All", id: 0 };
          this.$axios
            .get(`/maliyet/proforma/currency/${date.year}/${date.month}`)
            .then((res) => {
              this.profitProformaTl =
                this.getReportsMekmarAyoListTotal.proforma * res.data.currency;
            });
    ...
    getReportsMekmarAyoMonthList() {
      ...
      api
        ...
        .then((response) => {
          ...
          this.$axios
            .get(
              `/maliyet/proforma/currency/${this.selectedYear.Yil}/${this.selectedMonth.Ay}`
            )
            .then((res) => {
              this.profitProformaTl =
                this.getReportsMekmarAyoListTotal.proforma * res.data.currency;
