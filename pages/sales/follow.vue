<template>
  <div class="container">
    <div class="row">
      <followList
        @followDetail="followDetail($event)"
        v-for="item of getFollowList"
        :key="item.FirmaAdi"
        :follow="item"
      />
    </div>
    <Dialog :visible.sync="follow_dialog" modal :header="customer">
      <div class="row container">
        <div class="col">
          <button type="button" class="btn btn-outline-success w-100" @click="newForm">
            New
          </button>
        </div>
        <div class="col">
          <button type="button" class="btn btn-outline-primary w-100">Unfollow</button>
        </div>
      </div>
      <followForm
        :detailFollow="getFollowDetail"
        @follow_detail_data="followDetailData($event)"
      />
    </Dialog>
    <Dialog
      :visible.sync="follow_form_detail_dialog"
      :header="follow_form_detail_header"
      modal
      :breakpoints="{ '1199px': '75vw', '575px': '90vw' }"
    >
      <followFormDetail
        :followDetail="!getFollowDetailNewButton ? getFollowDetailData : followFormModel"
        @closed_follow_dialog="follow_form_detail_dialog = false"
      />
    </Dialog>
  </div>
</template>
