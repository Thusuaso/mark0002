<template>
    <div class="container">
        <div >
            <representativeList :list="getRepresentativeList" :orderer="getTotalRepresentative" :operation="getTotalOperation" @representative_form_emit="representativeFormEmit($event)"/>
        </div>
        <Dialog :visible.sync="representative_form_dialog" modal :header="representativeOrderNo">
...
export default {
    computed:{
      ...mapGetters(['getRepresentativeList','getTotalRepresentative','getTotalOperation','getUserList'])  
    },
    data(){
        return{
