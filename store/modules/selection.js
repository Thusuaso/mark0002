...
const actions = {
  ...
  setSelectionProductsList(vuexContext) {
    ...
    this.$axios.get("/selection/production/list").then((response) => {
      if (response.data) {
        vuexContext.commit("setSelectionProductsList", response.data.data);
        vuexContext.commit(
          "setSelectionProductList",
          response.data.data.mekmerList
        );
        vuexContext.commit(
          "setSelectionProductTotalList",
          response.data.data.mekmerList
        );
  ...
  setSelectionProductionTotal(vuexContext) {
    this.$axios.get("/selection/production/total").then((response) => {
  ...
  setSelectionProductionMekmerButton(vuexContext) {
    vuexContext.commit(
      "setSelectionProductList",
      vuexContext.state.productsList.mekmerList
    );
    vuexContext.commit(
      "setSelectionProductTotalList",
      vuexContext.state.productsList.mekmerList
  ...
  setSelectionProductionDisButton(vuexContext) {
    vuexContext.commit(
      "setSelectionProductList",
      vuexContext.state.productsList.disList
    );
    vuexContext.commit(
      "setSelectionProductTotalList",
      vuexContext.state.productsList.disList
  ...
  setSelectionProductionMekmerDisButton(vuexContext) {
    vuexContext.commit(
      "setSelectionProductList",
      vuexContext.state.productsList.mekmerDisList
    );
    vuexContext.commit(
      "setSelectionProductTotalList",
      vuexContext.state.productsList.mekmerDisList
  ...
  setSelectionProductionBulunamadiButton(vuexContext) {
    vuexContext.commit(
      "setSelectionProductList",
      vuexContext.state.productsList.bulunamadiList
    );
    vuexContext.commit(
      "setSelectionProductTotalList",
      vuexContext.state.productsList.bulunamadiList
  ...
  setSelectionProductionProductsList(vuexContext, po) {
    this.$axios.get(`/order/products/${po}`).then((response) => {
      vuexContext.commit(
  ...
  setSelectionProductionCrateNoIn(vuexContext) {
    this.$axios.get("/selection/production/crateno/in").then((response) => {
      vuexContext.commit("setSelectionProductionCrateNo", response.data.no);
    });
  },
  setSelectionProductionCrateNoOut(vuexContext) {
    this.$axios.get("/selection/production/crateno/out").then((response) => {
      vuexContext.commit("setSelectionProductionCrateNo", response.data.no);
    });
  },
  setSelectionProductionSave(vuexContext, product) {
    this.$axios.post("/selection/production/save", product).then((response) => {
  ...
  setSelectionProductionDelete(vuexContext, crateno) {
    this.$axios
      .delete(`/selection/production/delete/${crateno}`)
      .then((response) => {
        if (response.data.status) {
          vuexContext.dispatch("setSelectionProductionUpdateList");
          //  this.$socket.socketIO.emit('production_update_emit');
  ...
  setSelectionProductionUpdate(vuexContext, product) {
    this.$axios
      .put("/selection/production/update", product)
      .then((response) => {
        if (response.data.status) {
          vuexContext.dispatch("setSelectionProductionUpdateList");
          // this.$socket.socketIO.emit('production_update_emit');
  ...
  setSelectionProductionCrateSizeList(vuexContext) {
    vuexContext.dispatch("setBeginLoadingAction");
    this.$axios.get("/selection/production/cratesize").then((response) => {
      vuexContext.commit(
        "setSelectionProductionCrateSizeList",
        response.data.cratesize
      );
  ...
  setSelectionProductionCrateSizeSave(vuexContext, cratesize) {
    this.$axios
      .post("/selection/production/cratesize/save", cratesize)
      .then((response) => {
        if (response.data.status) {
          vuexContext.commit(
            "setSelectionProductionCrateSizeSave",
            response.data.cratesize
          );
  ...
  setSelectionProductionCrateSizeUpdate(vuexContext, cratesize) {
    this.$axios
      .put("/selection/production/cratesize/update", cratesize)
      .then((response) => {
        if (response.data.status) {
          vuexContext.commit(
            "setSelectionProductionCrateSizeUpdate",
            cratesize
          );
          vuexContext.dispatch("setSelectionProductionCrateSizeList");
          this.$toast.success("Başarıyla Güncellendi.");
  ...
  setSelectionProductionCrateSizeDelete(vuexContext, id) {
    this.$axios
      .delete(`/selection/production/cratesize/delete/${id}`)
      .then((response) => {
        if (response.data.status) {
          vuexContext.commit(
            "setSelectionProductionCrateSizeDelete",
            response.data.id
          );
          vuexContext.dispatch("setSelectionProductionCrateSizeList");
...
const mutations = {
  ...
  setSelectionProductionCrateSizeDelete(state, cratesize) {
    const index = state.productionCrateSizeList.findIndex(
      (x) => x.Id === cratesize.Id
    );
