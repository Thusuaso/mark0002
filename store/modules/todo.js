...
const actions = {
  setTodos(vuexContext) {
    this.$axios.get("/sales/todos/list").then((response) => {
      vuexContext.commit("setTodos", response.data);
    });
  },
  setTodoStatusChange(vuexContext, id) {
    this.$axios.put(`/sales/todos/change/status/${id}`).then((response) => {
  ...
  setTodoUpdate(vuexContext, todo) {
    this.$axios.put("/sales/todos/update", todo).then((response) => {
      if (response.data.status) {
  ...
  setTodoDelete(vuexContext, id) {
    this.$axios.delete(`/sales/todos/delete/${id}`).then((response) => {
      if (response.data.status) {
        const username = Cookies.get("username");
        vuexContext.dispatch("setTodos");
        vuexContext.dispatch("setTodosMainList", username);
  ...
  setToDoListByUsername(vuexContext, username) {
    this.$axios
      .get(`/todo/main/list/by/username/${username}`)
      .then((response) => {
  ...
  setTodoSave(vuexContext, todo) {
    this.$axios.post("/todo/by/username/save", todo).then((response) => {
      if (response.data.status) {
  ...
  setTodoDone(vuexContext, todo) {
    this.$axios.post(`/todo/by/username/done`, todo).then((response) => {
      if (response.data.status) {
  ...
  setTodosMainList(vuexContext, username) {
    this.$axios.get(`/sales/todos/main/list/${username}`).then((response) => {
      vuexContext.commit("setTodosMainList", response.data.list);
    });
  },
  setSalesTodoMainDone(vuexContext, id) {
    this.$axios.get(`/todo/main/list/change/done/${id}`).then((response) => {
      if (response.data.status) {
        this.$toast.success("Görev Tamamlandı.");
        vuexContext.dispatch("setTodosMainList", Cookies.get("username"));
      } else {
  ...
  setSalesTodoMainSeen(vuexContext, id) {
    this.$axios.get(`/todo/main/list/change/seen/${id}`).then((response) => {
      if (response.data.status) {
  ...
  setSalesTodoMainChangeQueue(vuexContext, todos) {
    this.$axios.post("/todo/main/change/queue", todos).then((response) => {
      if (response.data.status) {
  ...
  setTodoNotSeen(vuexContext, id) {
    this.$axios
      .get(`/todo/main/list/change/not/seen/${id}`)
      .then((response) => {
        if (response.data.status) {
