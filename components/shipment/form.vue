    ...
    <div class="row mb-4">
      ...
      <div class="col-3">
        <span class="p-float-label">
          <InputText
            id="orderamount"
            v-model="getShipmentAmount.order"
            disabled
          />
      ...
      <div class="col-3">
        <span class="p-float-label">
          <InputText
            id="forwardamount"
            v-model="getShipmentAmount.production"
            disabled
          />
...
export default {
  ...
  methods: {
    ordersSelected(event) {
      ...
      this.$store
        .dispatch("setShipmentOrderControl", event)
        .then((response) => {
          if (
            response.TeslimTurID == 5 ||
            response.TeslimTurID == 6 ||
            response.TeslimTurID == 7
          ) {
            this.save_button_disabled = false;
            this.sending_crate_button_disabled = false;
          } else {
            if (
              response.NavlunSatis == 0 ||
              response.NavlunSatis == null ||
              response.NavlunSatis == "" ||
              response.NavlunSatis == undefined
            ) {
              this.$toast.error(
                "Navlun satış bilgisi girilmeli.Girdikten Sonra Sayfayı Yenileyiniz!"
              );
              this.save_button_disabled = false;
              this.sending_crate_button_disabled = false;
            } else {
    ...
    __getPriceControl(payload) {
      const supplierName = this.getSupplierList.find(
        (x) => x.ID == payload.TedarikciID
      ).FirmaAdi;
      if (
        payload.SatisFiyati == 0 ||
        payload.SatisFiyati == undefined ||
        payload.SatisFiyati == null ||
        payload.SatisFiyati == ""
      ) {
        this.$toast.error(
          `${supplierName} ürünlerine satış fiyati girilmeli.Girdikten Sonra Sayfayı Yenileyiniz!`
        );
        this.save_button_disabled = false;
        this.sending_crate_button_disabled = false;
      } else {
      ...
      if (this.getOrderProductionProductNormalList.length == 0) {
        ...
        if (!this.selectedOrder) {
          ...
          if (!this.forwardingDate) {
            this.$toast.error(
              "Sevkiyat tarihi girilmemiş, tekrar deneyiniz..."
            );
            this.save_button_disabled = false;
          } else {
            if (
              ...
              if (
                ...
                this.forwardingstatus == " " ||
                this.forwardingstatus == null ||
                this.forwardingstatus == undefined
              ) {
                if (
                  confirm(
                    "Sevk türü seçilmemiş yine de sevk etmek istiyor musunuz?"
                  )
                ) {
                  ...
                  const datas = {
                    ...
                    YuklemeTarihi: convertDate.dateToString(
                      this.forwardingDate
                    ),
        ...
        this.$toast.error(
          "Gönderilmemiş Ürünler Mevcut. Kontrol edip tekrar dene..."
        );
      }
    },
    sendingCrate() {
      this.$store.dispatch(
        "setShipmentSendingProduction",
